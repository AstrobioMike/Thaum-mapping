...
do
    ...
    fasterq-dump --split-spot --split-files ${acc}/${acc}.sra >> ${mapping_dir}/${sample}-dl-and-mapping.log 2>&1
...
# setting variables that hold all forward and reverse reads, comma-delimited so can be passed to bowtie2 together
R1=$(echo $list_of_run_accs | sed 's/,/.sra_1.fastq,/g' | sed 's/$/.sra_1.fastq/')
R2=$(echo $list_of_run_accs | sed 's/,/.sra_2.fastq,/g' | sed 's/$/.sra_2.fastq/')

# mapping command
map_command="bowtie2 --mm -q -x ${bowtie2_db} -1 ${R1} -2 ${R2} -p ${num_threads} --no-unal 2> ${mapping_dir}/${sample}-mapping-info.txt | samtools view -b | samtools sort -@ ${num_threads} > ${mapping_dir}/${sample}.bam 2> /dev/null"
  # writing to log file
